...
public class HttpChatAction extends HttpAction {
	...
	protected String doInBackground(Void... params) {
		try {
		this.response = MainActivityBotLIbre.connection.chat(this.config);
	...
	protected void onPostExecute(String xml) {
		...
		try {
			MainActivityBotLIbre.conversation = this.response.conversation;
			...
			if (MainActivityBotLIbre.sound && this.response.avatarActionAudio != null && this.response.avatarActionAudio.length() > 0) {
				// Action audio
				activity.playAudio(this.response.avatarActionAudio, false, true, true);
			}
			if (MainActivityBotLIbre.sound && this.response.avatarAudio != null && this.response.avatarAudio.length() > 0) {
				activity.music = true;
				// Background audio
			...
			if (!MainActivityBotLIbre.disableVideo && !activity.videoError && this.response.isVideo()) {
				// Video avatar
