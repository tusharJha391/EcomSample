...
import com.allandroidprojects.sampleecomapp.actions.HttpGetVoiceAction;
import com.allandroidprojects.sampleecomapp.actions.HttpSaveVoiceAction;
import com.allandroidprojects.sampleecomapp.actions.HttpSpeechAction;
...
public class VoiceActivity extends LibreActivity implements TextToSpeech.OnInitListener {
	...
	public void onCreate(Bundle savedInstanceState) {
        ...
        HttpGetVoiceAction action = new HttpGetVoiceAction(this, (InstanceConfig)MainActivityBotLIbre.instance.credentials());
    	action.execute();
	}
	...
	public void resetView(){
		try {
			...
			ArrayAdapter adapter = new ArrayAdapter(this,
	                android.R.layout.simple_spinner_dropdown_item, MainActivityBotLIbre.voiceNames);
			voiceSpin.setAdapter(adapter);
			int index = Arrays.asList(MainActivityBotLIbre.voices).indexOf(MainActivityBotLIbre.voice.voice);
			if (index != -1) {
				voiceSpin.setSelection(index);
			}
			
			ArrayAdapter adapter1 = new ArrayAdapter(this, android.R.layout.simple_spinner_dropdown_item,
					MainActivityBotLIbre.voiceMods);
			voiceSpinMod.setAdapter(adapter1);
			index = Arrays.asList(MainActivityBotLIbre.voiceMods).indexOf(MainActivityBotLIbre.voice.mod);
			if (index != -1) {
				voiceSpinMod.setSelection(index);
			}

			CheckBox checkBox = (CheckBox) findViewById(R.id.deviceVoiceCheckBox);
			checkBox.setChecked(MainActivityBotLIbre.voice.nativeVoice);
			EditText text = (EditText) findViewById(R.id.pitchText);
			text.setText(MainActivityBotLIbre.voice.pitch);
			text = (EditText) findViewById(R.id.speechRateText);
			text.setText(MainActivityBotLIbre.voice.speechRate);
		} catch (Exception e) {
			MainActivityBotLIbre.error(e.getMessage(), e, this);
		}
	}
    ...
    public void onInit(int status) { 
        if (status == TextToSpeech.SUCCESS) {
            ...
            if (MainActivityBotLIbre.voice.language != null) {
            	locales.add(MainActivityBotLIbre.voice.language);
            }
            ...
            if (MainActivityBotLIbre.voice.language != null) {
            	spin.setSelection(locales.indexOf(MainActivityBotLIbre.voice.language));
            }
    ...
    public void test(View view) {
		...
		if (deviceVoiceCheckBox.isChecked()) {
	        ...
	        if (result == TextToSpeech.LANG_NOT_SUPPORTED) {
	            MainActivityBotLIbre.error("This Language is not supported", null, this);
	        }
