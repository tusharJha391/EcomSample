...
import com.allandroidprojects.sampleecomapp.actions.HttpConnectAction;
import com.allandroidprojects.sampleecomapp.actions.HttpFetchAction;
import com.allandroidprojects.sampleecomapp.actions.HttpFetchOrCreateAction;
import com.allandroidprojects.sampleecomapp.actions.HttpGetImageAction;
import com.allandroidprojects.sampleecomapp.actions.HttpGetTemplatesAction;
 ...
 * You do not need to use the MainActivityBotLIbre in your app, but it needs to be there so the other activities can access the shared data.
 ...
 * <p>
 * You can create an app to access a single bot, forum, or channel instance using this MainActivityBotLIbre class.
...
@SuppressLint("NewApi")
public class MainActivityBotLIbre extends LibreActivity {
    ...
    public static String applicationId = "1294855710680956329";
	...
	public static String WEBSITE = " http://www.botlibre.com/browse?id=25106962";
	public static final String WEBSITEHTTPS = " http://www.botlibre.com/browse?id=25106962";
    ...
    public static String launchInstanceId = "25106962";
	//public static String launchInstanceName = "Santa Bot"; // i.e. "Help Bot"
    public static String launchInstanceName = "ServiceBot";
	/**
	...
	public static MainActivityBotLIbre current;
	...
	public static void error(String message, Exception exception, Activity activity) {
		try {
			...
			MainActivityBotLIbre.showMessage(message, activity);
        ...
        builder.setMessage("Select Content Rating: ")
               .setPositiveButton("Select", new DialogInterface.OnClickListener() {
                   public void onClick(DialogInterface dialog, int id) {
                       //save data
                	   SharedPreferences.Editor editor = MainActivityBotLIbre.current.getPreferences(Context.MODE_PRIVATE).edit();
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (user == null) {
			...
			if (voice != null) {
				...
				MainActivityBotLIbre.voice = new VoiceConfig();
				MainActivityBotLIbre.voice.voice = voice;
				MainActivityBotLIbre.voice.language = language;
				MainActivityBotLIbre.voice.nativeVoice = Boolean.valueOf(nativeVoice);
				MainActivityBotLIbre.deviceVoice = MainActivityBotLIbre.voice.nativeVoice;
			}
			if (translate != null) {
				MainActivityBotLIbre.translate = true;
				MainActivityBotLIbre.customVoice = true;
				MainActivityBotLIbre.voice = new VoiceConfig();
				MainActivityBotLIbre.voice.language = language;
				MainActivityBotLIbre.voice.nativeVoice = Boolean.valueOf(nativeVoice);
		...
		if ((launchType == LaunchType.Bot) || (launchType == LaunchType.Channel)) {
			...
			setContentView(R.layout.activity_main_botlibre);
		}
	...
	public void resetView() {
		...
		if (spin != null) {
			ArrayAdapter adapter = new ArrayAdapter(this,
					android.R.layout.simple_spinner_dropdown_item, MainActivityBotLIbre.types);
			spin.setAdapter(adapter);
			spin.setSelection(Arrays.asList(MainActivityBotLIbre.types).indexOf(MainActivityBotLIbre.type));
		...
		if (MainActivityBotLIbre.user == null) {
			...
			HttpGetImageAction.fetchImage(this, MainActivityBotLIbre.user.avatar, findViewById(R.id.viewUserButton));
		}
		resetMenu(this.menu);
		resetLast();
	}
	...
	public void resetLast() {
		String type = MainActivityBotLIbre.defaultType;
		Spinner spin = (Spinner) findViewById(R.id.typeSpin);
		if (spin != null) {
		...
		if (type == null) {
			type = MainActivityBotLIbre.defaultType;
		}
	...
	public void openLast(View view) {
		...
		if (last == null) {
			MainActivityBotLIbre.showMessage("Invalid cookie", this);
			return;
		}
	...
	public void browseMyBot() {
		...
		config.name = "Bot " + MainActivityBotLIbre.user.user;
	...
	public static void changeLanguage(Activity activty, final DialogInterface.OnClickListener listener) {
		...
		ArrayAdapter adapter = new ArrayAdapter(activty,
				android.R.layout.simple_spinner_dropdown_item, MainActivityBotLIbre.languages);
		spin.setAdapter(adapter);
		int index = -1;
		
		if (MainActivityBotLIbre.voice != null && MainActivityBotLIbre.voice.language != null) {
			index = Arrays.asList(MainActivityBotLIbre.languages).indexOf(MainActivityBotLIbre.voice.language);
		}
		spin.setSelection(index);
		
		if (MainActivityBotLIbre.voice != null && MainActivityBotLIbre.voice.language != null){
			for (int i = 0; i < adapter.getCount(); i++) {
				if(MainActivityBotLIbre.languages[i].contains(MainActivityBotLIbre.voice.language + " - ")){
					spin.setSelection(i);
				}
			}
		}
		...
		dialog.setButton(AlertDialog.BUTTON_POSITIVE, "OK", new DialogInterface.OnClickListener() {
			public void onClick(DialogInterface dialog, int whichButton) {
				...
				if (lang.equals("Default")) {
					MainActivityBotLIbre.translate = false;
					MainActivityBotLIbre.customVoice = false;
					MainActivityBotLIbre.voice = new VoiceConfig();
					MainActivityBotLIbre.deviceVoice = false;
					...
					MainActivityBotLIbre.translate = true;
					if (MainActivityBotLIbre.voice == null) {
						MainActivityBotLIbre.voice = new VoiceConfig();
					}
					MainActivityBotLIbre.voice.language = lang.substring(0, 2);
					MainActivityBotLIbre.voice.nativeVoice = true;
					MainActivityBotLIbre.deviceVoice = true;
					MainActivityBotLIbre.customVoice = true;
