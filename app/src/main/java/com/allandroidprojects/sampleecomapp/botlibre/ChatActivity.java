...
import com.allandroidprojects.sampleecomapp.actions.HttpChatAction;
import com.allandroidprojects.sampleecomapp.actions.HttpFetchChatAvatarAction;
import com.allandroidprojects.sampleecomapp.actions.HttpGetImageAction;
import com.allandroidprojects.sampleecomapp.actions.HttpGetVideoAction;
...
public class ChatActivity extends LibreActivity implements TextToSpeech.OnInitListener, TextToSpeech.OnUtteranceCompletedListener, RecognitionListener {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (MainActivityBotLIbre.launchType == MainActivityBotLIbre.LaunchType.Bot) {
		...
		//For "scream" issue
		micLastStat = MainActivityBotLIbre.listenInBackground;
		
		getWindow().addFlags(WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON);
		
		this.instance = (InstanceConfig)MainActivityBotLIbre.instance;
		if (this.instance == null) {
			return;
		}
		/*if (MainActivityBotLIbre.showAds) {
		...
		if (!MainActivityBotLIbre.handsFreeSpeech){
			setMicIcon(false, false);
		} else if (!MainActivityBotLIbre.listenInBackground){
			setMicIcon(false, false);
		}
		
		//Last time will be saved for the MIC.
		if (MainActivityBotLIbre.listenInBackground && MainActivityBotLIbre.handsFreeSpeech){
		...
		if (MainActivityBotLIbre.translate) {
		...
		button.setOnClickListener(new View.OnClickListener() { 
			...
			public void onClick(View v) { 
				if (MainActivityBotLIbre.handsFreeSpeech) {
					...
					MainActivityBotLIbre.listenInBackground = !MainActivityBotLIbre.listenInBackground;
					
					//saving the boolean data of MainActivityBotLIbre.listeningInBackground
			    	SharedPreferences.Editor cookies = MainActivityBotLIbre.current.getPreferences(Context.MODE_PRIVATE).edit();
			    	cookies.putBoolean("listenInBackground", MainActivityBotLIbre.listenInBackground);
			    	cookies.commit();
					if (MainActivityBotLIbre.listenInBackground) {
						micLastStat = true;
					...
					intent.putExtra(RecognizerIntent.EXTRA_LANGUAGE_MODEL, MainActivityBotLIbre.voice.language);
					try {
		...
		GestureDetector.SimpleOnGestureListener listener = new GestureDetector.SimpleOnGestureListener() {
			...
			public boolean onDoubleTapEvent(MotionEvent event) {
				if (event.getAction() == MotionEvent.ACTION_UP) {
					boolean isVideo = !MainActivityBotLIbre.disableVideo && !videoError && response != null
							&& response.isVideo();
		...
		if (MainActivityBotLIbre.translate && MainActivityBotLIbre.voice != null) {
			config.language = MainActivityBotLIbre.voice.language;
		}
		if (MainActivityBotLIbre.disableVideo) {
			config.avatarFormat = "image";
		} else {
			config.avatarFormat = MainActivityBotLIbre.webm ? "webm" : "mp4";
		}
		config.avatarHD = MainActivityBotLIbre.hd;
		config.speak = !MainActivityBotLIbre.deviceVoice;
	...
	public Thread microphoneThread(Thread thread) {
		...
		if(MainActivityBotLIbre.listenInBackground && threadIsOn){
			return thread;
		}
	...
	public void flagResponse() {
        if (MainActivityBotLIbre.user == null) {
        	MainActivityBotLIbre.showMessage("You must sign in to flag a response", this);
        	return;
        }
        final EditText text = new EditText(this);
        MainActivityBotLIbre.prompt("Enter reason for flagging response as offensive", this, text, new DialogInterface.OnClickListener() {
	        public void onClick(DialogInterface dialog, int whichButton) {
	    		...
	    		ChatConfig config = new ChatConfig();
	    		config.instance = instance.id;
	    		config.conversation = MainActivityBotLIbre.conversation;
	    		config.speak = !MainActivityBotLIbre.deviceVoice;
	...
	public void submitCorrection() {
        final EditText text = new EditText(this);
        MainActivityBotLIbre.prompt("Enter correction to the bot's response (what it should have said)", this, text, new DialogInterface.OnClickListener() {
	        public void onClick(DialogInterface dialog, int whichButton) {
	            if (instance == null) {
	            	return;
	            }
	...
	public void submitChat() {
		...
		MainActivityBotLIbre.listenInBackground = false;
	}
	...
	public void toggleSound() {
		MainActivityBotLIbre.sound = !MainActivityBotLIbre.sound;
		resetToolbar();
	}
	public void toggleHandsFreeSpeech() {
		MainActivityBotLIbre.handsFreeSpeech = !MainActivityBotLIbre.handsFreeSpeech;
		if (!MainActivityBotLIbre.handsFreeSpeech) {
			stopListening();
		} else if (MainActivityBotLIbre.handsFreeSpeech) {
			beginListening();
		}
		SharedPreferences.Editor cookies = MainActivityBotLIbre.current.getPreferences(Context.MODE_PRIVATE).edit();
    	cookies.putBoolean("handsfreespeech", MainActivityBotLIbre.handsFreeSpeech);
    	cookies.commit();
	}
	public void toggleDisableVideo() {
		if (this.videoError) {
			this.videoError = false;
			MainActivityBotLIbre.disableVideo = false;
		} else {
			MainActivityBotLIbre.disableVideo = !MainActivityBotLIbre.disableVideo;
		}
	}
	...
	public void toggleDeviceVoice() {
		MainActivityBotLIbre.deviceVoice = !MainActivityBotLIbre.deviceVoice;
	}
	...
	public void resetMenu() {
		...
		this.menu.findItem(R.id.menuSound).setChecked(MainActivityBotLIbre.sound);
		this.menu.findItem(R.id.menuDeviceVoice).setChecked(MainActivityBotLIbre.deviceVoice);
		this.menu.findItem(R.id.menuHandsFreeSpeech).setChecked(MainActivityBotLIbre.handsFreeSpeech);
		this.menu.findItem(R.id.menuDisableVideo).setChecked(MainActivityBotLIbre.disableVideo || this.videoError);
		this.menu.findItem(R.id.menuHD).setChecked(MainActivityBotLIbre.hd);
		this.menu.findItem(R.id.menuWebm).setChecked(MainActivityBotLIbre.webm);
	}

	public void changeLanguage(View view) {
		MainActivityBotLIbre.changeLanguage(this, new DialogInterface.OnClickListener() {
			@Override
...
//			return true;
		...
		case R.id.menuFlag:
			...
			return true;
		case R.id.menuHD:
			MainActivityBotLIbre.hd = !MainActivityBotLIbre.hd;
			return true;
		case R.id.menuWebm:
			MainActivityBotLIbre.webm = !MainActivityBotLIbre.webm;
			return true;
	...
	public void onResume() {
		...
		MainActivityBotLIbre.searchingPosts = false;
		if (MainActivityBotLIbre.browsing && (MainActivityBotLIbre.instance instanceof AvatarConfig)) {
			if (MainActivityBotLIbre.user == null || MainActivityBotLIbre.user.type == null || MainActivityBotLIbre.user.type.isEmpty() || MainActivityBotLIbre.user.type.equals("Basic")) {
				//MainActivityBotLIbre.showMessage("You must upgrade to get access to this avatar", this);
				//super.onResume();
				//return;
			}
			this.avatar = (AvatarConfig)MainActivityBotLIbre.instance;
		...
		MainActivityBotLIbre.browsing = false;
		if ((MainActivityBotLIbre.instance instanceof InstanceConfig) && MainActivityBotLIbre.instance.id.equals(this.instance.id)) {
			this.instance = (InstanceConfig)MainActivityBotLIbre.instance;
		} else {
			MainActivityBotLIbre.instance = this.instance;
		}
	...
	public void onInit(int status) {
		...
		if (status == TextToSpeech.SUCCESS) {
			...
			Locale locale = null;
			VoiceConfig voice = MainActivityBotLIbre.voice;
	...
	public void response(final ChatResponse response) {
		...
		try {
			...
			boolean talk = (text.trim().length() > 0) && (MainActivityBotLIbre.deviceVoice || (this.response.speech != null && this.response.speech.length() > 0));
			if (MainActivityBotLIbre.sound && talk) {
				if (!MainActivityBotLIbre.disableVideo && !videoError && this.response.isVideo() && this.response.isVideoTalk()) {
					...
					videoView.setOnPreparedListener(new OnPreparedListener() {
						...
						public void onPrepared(MediaPlayer mp) {
							try {
								mp.setLooping(true);
								if (!MainActivityBotLIbre.deviceVoice) {
									// Voice audio
			...
			} else if (talk && (!MainActivityBotLIbre.disableVideo && !videoError && this.response.isVideo() && this.response.avatarTalk != null)) {
				videoView.setOnPreparedListener(new OnPreparedListener() {
					@Override
					public void onPrepared(MediaPlayer mp) {
						mp.setLooping(false);
		...
		if(micLastStat){
			MainActivityBotLIbre.listenInBackground = true;
		}
	}
	...
	public void playVideo(String video, boolean loop) {
		...
		try {
			...
			if (videoUri == null) {
				videoUri = Uri.parse(MainActivityBotLIbre.connection.fetchImage(video).toURI().toString());
			}
	...
	public void cycleVideo(final ChatResponse response) {
		...
		try {
			...
			if (videoUri == null) {
				videoUri = Uri.parse(MainActivityBotLIbre.connection.fetchImage(avatar).toURI().toString());
			}
			videoView.setVideoURI(videoUri);
			videoView.start();
		} catch (Exception exception) {
	...
	public MediaPlayer playAudio(String audio, boolean loop, boolean cache, boolean start) {
		try {
			...
			if (audioUri == null) {
				audioUri = Uri.parse(MainActivityBotLIbre.connection.fetchImage(audio).toURI().toString());
			}
	...
	public void onUtteranceCompleted(String utteranceId) {
		...
		try {
			if (!MainActivityBotLIbre.disableVideo && !videoError && this.response.isVideo()) {
	...
	private void muteMicBeep(boolean mute) {
		debug("muteMicBeep:" + mute + ":" + MainActivityBotLIbre.volume);
		AudioManager audioManager = (AudioManager) getSystemService(Context.AUDIO_SERVICE);	
		if (mute) {
			...
			audioManager.setStreamVolume(AudioManager.STREAM_MUSIC, MainActivityBotLIbre.volume, 0);
		}
	}
	...
	private void setStreamVolume() {
		...
		if (volume != 0) {
			...
			MainActivityBotLIbre.volume = volume;
		} 
	}
	@TargetApi(23)
	private void beginListening() {
		...
		try {
			if(!MainActivityBotLIbre.handsFreeSpeech){return;}
			if (MainActivityBotLIbre.handsFreeSpeech) {
				muteMicBeep(true);
				isListening = true;
			}
		...
		Intent intent = new Intent(RecognizerIntent.ACTION_RECOGNIZE_SPEECH);
		if (MainActivityBotLIbre.offlineSpeech) {
			intent.putExtra(RecognizerIntent.EXTRA_LANGUAGE_MODEL, MainActivityBotLIbre.voice.language);
			...
			if (MainActivityBotLIbre.voice.language != null && !MainActivityBotLIbre.voice.language.isEmpty()) {
				intent.putExtra(RecognizerIntent.EXTRA_LANGUAGE_MODEL, MainActivityBotLIbre.voice.language);
				if (!this.failedOfflineLanguage) {
					intent.putExtra(RecognizerIntent.EXTRA_LANGUAGE, MainActivityBotLIbre.voice.language);
				}
			} else {
